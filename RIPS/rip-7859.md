...
[RIP-7755](./rip-7755.md), enforcing L2 consistency checks such as [RIP-7789](https://ethereum-magicians.org/t/rip-7789-cross-rollup-contingent-transactions/21402), and verifying reads from the L1 such as
[RIP-7728](./rip-7728.md) and [ERC-3668](https://eips.ethereum.org/EIPS/eip-3668). In general, it is useful to any
contract on the L2 that needs to verify arbitrary L1 state, storage, transaction, or receipt data inside the L2.
...
In defining this interface we also define _what_ information about the `L1_ORIGIN` must be exposed. This information
is called the `L1_ORIGIN_INFO_HASH` and is a keccak256 hash of a subset of the `L1_ORIGIN`'s block header fields defined
as:

`L1_ORIGIN_INFO_HASH = keccak256([L1_ORIGIN.block.height, L1_ORIGIN.block.stateRoot, L1_ORIGIN.block.transactionRoot, L1_ORIGIN.block.receiptRoot])`

This hash is exposed with two methods on the contract, one for accessing the current `L1_ORIGIN_INFO_HASH` and one for
accessing a `L1_ORIGIN_INFO_HASH` at a specific L1 block height.

```solidity
interface L1OriginSource {
    function getL1OriginInfoHash() external view returns (bytes32 l1OriginInfoHash);

    function getL1OriginInfoHashAt(uint256 height) external view returns (bytes32 l1OriginInfoHash);
}
```

For this latter method we additionally specify that the `L1_ORIGIN_INFO_HASH` be available for _at least_ the most
...
### Hash of a subset of execution block fields instead of beacon block root or full execution block hash

`keccak256([L1_ORIGIN.block.height, L1_ORIGIN.block.stateRoot, L1_ORIGIN.block.transactionRoot, L1_ORIGIN.block.receiptRoot])`
is exposed in order to reduce gas costs relative to using either beacon block roots or the hash of a full execution block.

The preimage to this hash is the 128 byte concatenation of the four fields
`[L1_ORIGIN.block.height, L1_ORIGIN.block.stateRoot, L1_ORIGIN.block.transactionRoot, L1_ORIGIN.block.receiptRoot]`.
This is smaller than the 708 byte preimage of a full execution header and smaller than an SSZ proof for a Merkle tree
with depth of 3 or greater (160+ bytes). SSZ Merkle trees on average have a depth of 7 or 8 levels.

The tradeoff is that the contents of this ring buffer can not be used to verify other fields of the L1 execution header
or beacon block. We believe these fields are the most valuable fields for verifying information about the L1 as they
are sufficient for verifying claims about all L1 state, storage, transactions, receipts, and logs at a given block
height.

### Access to historic L1 origin information

In this RIP we propose an interface that allows for access not to only the current `L1_ORIGIN_INFO_HASH` but also
the `L1_ORIGIN_INFO_HASH` for the last 8190 `L1_ORIGIN`s. This provides a view of the latest 8191 `L1_ORIGIN`s, matching
...
One approach would be to use ring buffer construction as in EIP-4788 and EIP-2935 where this ring buffer stores
the `L1_ORIGIN_INFO_HASH` instead of beacon block roots (EIP-4788) or full execution header hashes (EIP-2935). This
buffer can be updated with a system call or transaction at the top of the L2 block using the L1 context available to the
sequencer as it processes deposits from the `L1_ORIGIN` into the nascent L2 block.
...
current `L1_ORIGIN`. To support the interface defined here the individual field values can be hashed together.
