...
[EIP-4788](https://eips.ethereum.org/EIPS/eip-4788) with minor modification to store a hash of a subset of L1 execution
block header fields instead of beacon block roots.
...
useful for L1<->L2 and L2<->L2 interoperability and composability. Some specific examples of where this is useful
include:
...
The **L1 origin storage contract** can be deployed as a
[pre-deployed contract](https://github.com/ethereum-optimism/specs/blob/main/specs/protocol/predeploys.md).
...
3. In an L2 block includes transactions derived from L1 inputs the L1 origin for that L2 block must be the latest
...
The **L2 history storage contract** can be deployed as a
[pre-deployed contract](https://github.com/ethereum-optimism/specs/blob/main/specs/protocol/predeploys.md).
